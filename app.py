...
def _render_sidebar():
    ...
    if _get_user():
        ...
        if level:
            ...
            st.sidebar.markdown(f"**Status:** {emoji} {level}  \n_{label}_")
    ...
    with st.sidebar.expander("⚠️ Medical Disclaimer", expanded=False):
        st.markdown(
            """
**This app is for informational support only — it does not replace professional medical advice.**

- This app does not diagnose, prescribe, or replace your doctor or dietitian
- If your glucose is repeatedly above **180 mg/dL**, consult your doctor
- **Signs of low blood sugar:** shakiness, sweating, dizziness, confusion → seek help immediately
- **Signs of high blood sugar:** frequent urination, excessive thirst, blurred vision → consult your doctor
- Always follow your healthcare provider's instructions regarding diet and medication
- In an emergency, call your local emergency services immediately
            """
        )
    st.sidebar.caption("v1.0.0 · HEC GenAI Hackathon")
...
if "user_key" not in st.session_state:
    ...
    with col1:
        ...
        st.caption("Include your country code e.g. +92, +44, +1")
        ...
        if st.button("Continue →", type="primary", use_container_width=True):
            ...
            with st.spinner("Logging in..."):
                ...
                try:
                    ...
                    st.error("Unable to connect to the database. Please refresh and try again.")
                    st.stop()
    ...
    with col2:
        ...
        **What this app does:**
        ...
        - 💡 Gives AI-powered healthy food swap suggestions

        **Privacy:** Your phone number is never stored —
        only a secure one-way hash is used to identify you.

        **Important:** This app supports healthy habits.
        It does not replace your doctor or dietitian.
        """)
...
with tabs[0]:
    ...
    if _blocked():
        st.error(
            "🔴 **Your readings require clinical attention.** "
            "Please consult a doctor or dietitian before making dietary changes. "
            "The meal plan is unavailable until your readings are within a safe range."
        )
    else:
        if _triage_level() is None:
            st.info(
                "💡 **Complete your profile** (My Profile tab) to get a plan tailored to your health conditions. "
                "Showing a general plan for now."
            )
        ...
        if plan:
            for day in plan:
                ...
                with st.expander(f"📅 Day {day['day']}  —  ~{day_carbs:.0f}g carbs total"):
                    ...
                    if st.button("💡 Suggest healthy swaps", key=f"swaps_{day['day']}"):
...
with tabs[1]:
    ...
    st.caption("Your details personalise your meal plan and help us check whether this app is appropriate for your situation.")
    ...
    with st.expander("🔬 Advanced health data (optional)", expanded=False):
        st.caption("Fill in what you know from recent lab results or readings. Leave as 0 if unsure.")
        ...
        st.markdown("**Recent fasting glucose readings** (if you don't have your HbA1c)",
    ...
    if st.button("💾 Save & personalise my plan", type="primary"):
        ...
        st.success("✅ Profile saved! Your meal plan has been updated.")
    ...
    if "triage_level" in st.session_state:
        ...
        if level == "GREEN":
            st.success(
                f"{emoji} **All clear** — Your readings look stable. "
                "All features are unlocked. Keep up the consistency!"
            )
        elif level == "AMBER":
            st.warning(
                f"{emoji} **Proceed with care** — You can use the app, but we strongly recommend "
                "discussing your dietary plan with a doctor or dietitian."
            )
        else:
            st.error(
                f"{emoji} **Please see a clinician** — Your readings suggest that dietary changes alone "
                "may not be enough. Please seek professional medical advice before proceeding."
            )
        ...
        if st.session_state.get("family_history"):
            st.info(
                ...
                "This is used to personalise your plan. Always follow clinician guidance for specific targets."
            )
...
with tabs[2]:
    ...
    if not user:
        ...
        st.error(
            ...
            "Please consult a clinician before using this feature."
        )
    else:
        ...
        if followed == "Yes":
            ...
            if st.button("💾 Save + get suggestions", type="primary"):
                if not actual.strip():
                    st.warning("Please describe what you ate to get personalised suggestions.")
                else:
                    with st.spinner("Saving and generating personalised tips..."):
                    ...
                    st.success("✅ Saved! Here are some healthier ways to approach that choice next time:")
                    for t in tips:
                        st.write("•", t)
                    st.caption("These suggestions are for general dietary awareness only — not medical advice.")
...
with tabs[3]:
    ...
    if not user:
        ...
        st.error(
            "🔴 Glucose logging is unavailable. "
            "Please consult a clinician before using this feature."
        )
    else:
        col1, col2 = st.columns(2)
        ...
        with col2:
            ...
            st.caption(
                f"Reference ranges (mg/dL):  \n"
                f"🟡 Low < {TRIAGE['hypo']}  |  "
                f"🟢 Fasting target: 80–130  |  "
                f"🔴 Very high ≥ {TRIAGE['very_high']}"
            )

            if value >= TRIAGE["very_high"]:
                st.error("⚠️ This reading is very high. If you feel unwell, seek medical attention immediately.")
...
with tabs[4]:
    ...
    st.caption("Track your patterns over time. Share this with your doctor or dietitian for better guidance.")
    ...
    if not user:
        ...
        with col1:
            ...
            if checkins:
                ...
                st.dataframe(
                    cdf[["date", "followed_plan"]].rename(columns={"followed_plan": "Followed plan?"}),
                    use_container_width=True
                )
            else:
                st.info(
                    "👋 No check-ins recorded yet. "
                    "Use the **Daily Check-In** tab each day to start tracking your adherence."
                )
        ...
        with col2:
            ...
            if rows:
                ...
                if high_count:
                    st.warning(f"⚠️ {high_count} very high reading(s) detected. If persistent or you feel unwell, consult your doctor.")
                if low_count:
                    st.warning(f"⚠️ {low_count} low reading(s) detected. Discuss with your clinician.")
            else:
                st.info(
                    "🩸 No glucose readings yet. "
                    "Use the **Log Glucose** tab to start tracking your blood sugar."
                )
        ...
        if rows:
            ...
            ax.axhline(TRIAGE["very_high"], color="red",    linestyle="--", alpha=0.5, label=f"Very high ({TRIAGE['very_high']} mg/dL)")
            ax.axhline(TRIAGE["hypo"],      color="orange", linestyle="--", alpha=0.5, label=f"Low ({TRIAGE['hypo']} mg/dL)")
            ax.set_ylabel("mg/dL")
            ax.legend(fontsize=8)
            plt.xticks(rotation=30)
            plt.tight_layout()
            st.pyplot(fig)
            st.caption("This chart is for personal awareness only. Consult your doctor to interpret your readings.")
